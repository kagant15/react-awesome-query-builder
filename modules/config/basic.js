...
const widgets = {
  ...
  time: {
    ...
    elasticSearchFormatValue: (queryType, value, operator, fieldName)=>{
      return {
        script: {
          script: {
            source: `doc[${fieldName}][0].getHour() >== params.min && doc[${fieldName}][0].getHour() <== params.max`,
            params:{
              min: value[0],
              max: value[1],
            }
